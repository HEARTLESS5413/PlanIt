...
function initApp() {
  if (isLoggedIn()) {
    ...
    // Show mobile nav when app is active
    const mobileNav = document.getElementById('mobile-nav');
    if (mobileNav) mobileNav.style.display = '';
  } else {
    showPage('auth-page');
    // Hide mobile nav on auth page
    const mobileNav = document.getElementById('mobile-nav');
...
function setupMobileNav() {
  ...
  // Also sync when sidebar nav is clicked
  sidebarNavItems.forEach((item) => {
    const originalHandler = item.onclick;
    item.addEventListener('click', () => {
      syncNavActive(item.dataset.view);
    });
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Show loading screen for 2 seconds, then initialize
  setTimeout(() => {
    initApp();
